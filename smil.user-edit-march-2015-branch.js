...
function initSMIL() {
	...
	/** new test for internet explorer
	 */

	var impl = document.implementation;
	if (impl.hasFeature(smil3ns, "3.0")) {
		//	return;
	}


	/** Dummy test ... or checking for nested svg?
	 */
...
function smile(animating) {
	...
	var elms_animate = animating.getElementsByTagName("animate");
	var elms_animate_motion = animating.getElementsByTagName("animateMotion");
	var elms_animate_transform = animating.getElementsByTagName("animateTransform");
	var elms_animate_color = animating.getElementsByTagName("animateColor");
	var elms_set = animating.getElementsByTagName("set");
	var i; // for loop iterator
	var l; // array length


	if (elms_animate.length > 0) {
		for (i = 0, l = elms_animate.length; i < l; ++i) {
			animates.push(elms_animate.item(i));
		}
	}

	if (elms_animate_motion.length > 0) {
		for (i = 0, l = elms_animate_motion.length; i < l; ++i) {
			animates.push(elms_animate_motion.item(i));
		}
	}

	if (elms_animate_transform.length > 0) {
		for (i = 0, l = elms_animate_transform.length; i < l; ++i) {
			animates.push(elms_animate_transform.item(i));
		}
	}

	if (elms_animate_color.length > 0) {
		for (i = 0, l = elms_animate_color.length; i < l; ++i) {
			animates.push(elms_animate_color.item(i));
		}
	}

	if (elms_set.length > 0) {
		for (i = 0, l = elms_set.length; i < l; ++i) {
			animates.push(elms_set.item(i));
		}
	}

	for (i = 0, l = animates.length; i < l; ++i) {
		var anim = animates[i];
...
Animator.prototype = {
	...
	valueAt : function(percent) {
		var tValues = this["animVals"], i=0, len=0, parts=0;
		if (percent==1)
